<template>
    <div>
        ...
        <div class="shadow-7">
            <div class="q-pa-sm">
                Brought to you by AnimeNewsNetwork
            </div>
                    ...
                    <div class="col-lg-12">
                        <div class="q-pa-md">
                            <div class="bitSmaller">
                                ...
                                <a style="max-width:auto;" class="readMore artTitle flex" target="_blank" :href="article.link">{{ article.title }}</a>
                                <q-btn color="blue-6" @click="modalArticle(article.link)" size="sm" label="in-view"></q-btn>
                            </div>
...
@Component
export default class ANNNews extends Vue {
    ...
    public async mounted() {
        try {
            const response = (await OtherNewsService.getANNNews()).data;
            ...
            response.forEach((element) => {
                ...
                element.link = element.link.replace('http:', 'https:');
            ...
            this.Articles = response.slice(0, 9);
        } catch (error) {
            console.log(error);
        }
    }
